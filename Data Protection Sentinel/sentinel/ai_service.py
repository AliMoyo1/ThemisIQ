"""
Data Protection Sentinel — AI service layer.
Supports Anthropic, OpenAI, and Google Gemini.
Covers all regulation frameworks and all AI features.
"""
import os
import json
import requests
...
ANTHROPIC_MDL = os.getenv("ANTHROPIC_MODEL", "claude-opus-4-6")
...
def _call_anthropic(system, user, max_tokens=4096):
    headers = {
...
def _call_openai(system, user, max_tokens=4096):
...
def _call_gemini(system, user, max_tokens=4096):
...
def call_ai(system, user, max_tokens=4096):
...
REGULATION_META = {
    ...
    "USA CCPA/CPRA": {
        ...
        "key_articles": "§ 1798.100–1798.199 CCPA; CPRA amendments on sensitive personal information",
    ...
    "Brazil LGPD": {
        "full": "Brazil Lei Geral de Proteção de Dados Pessoais — Law No. 13,709/2018",
...
LEGAL_BASES = {
    ...
    "Saudi PDPL": [
        ...
        "Contract conclusion or execution",
        "Protection of vital interests",
        "Data made publicly available by the subject",
        "Judicial and security purposes",
    ],
    "Qatar DPL": [
        ...
        "Legal obligation",
        "Vital interests",
        "Public interest or government task",
        "Legitimate interests",
    ],
    "Canada PIPEDA": [
        "Expressed consent",
    ...
    "USA CCPA/CPRA": [
        ...
        "Opt-in (sensitive personal information)",
        "Business purpose necessity",
        "Service provider disclosure",
        "Legal obligation",
    ],
    "Brazil LGPD": [
        "Consent — Art. 7(I)",
        "Legal or regulatory compliance — Art. 7(II)",
        "Public policy execution — Art. 7(III)",
        "Research studies — Art. 7(IV)",
        "Contract execution — Art. 7(V)",
        "Judicial proceedings — Art. 7(VI)",
        "Life or physical safety protection — Art. 7(VII)",
    ...
    "India DPDP": [
        "Consent — s. 6",
        "Legitimate uses — s. 7 (state functions)",
        "Legal obligation",
        "Medical emergencies / epidemic response",
        "Employment purposes",
    ],
    "Singapore PDPA": [
        "Consent — Part IV",
        "Legitimate interests — Second Schedule",
    ...
    "Australia Privacy Act": [
        ...
        "Necessary for enforcement body functions",
    ...
    "Japan APPI": [
        ...
        "Necessary for compliance with laws",
    ...
    "South Korea PIPA": [
        "Consent — Art. 15(1)(1)",
        "Special laws/regulations — Art. 15(1)(2)",
        "Contract performance — Art. 15(1)(4)",
        "Vital interests — Art. 15(1)(5)",
        "Legitimate interests — Art. 15(1)(6)",
    ],
    "Canada Bill C-11": [
        "Consent (express or implied)",
        "Legitimate interests (with risk assessment)",
...
SPECIAL_CATEGORIES = [
    ...
    "Financial distress / insolvency data",
...
def ai_research(activity_type, regulation, context=""):
...
def ai_generate_full_dpia(dpia):
...
def ai_suggest_risks(activity, regulation, categories):
...
Return JSON: {{"risk_score": "low|medium|high|critical", "rationale": "2-3 sentence explanation", "dpia_required": true|false, "flags": ["flag1","flag2"]}}"""
    text, err = call_ai(system, user, max_tokens=500)
    if err:
        return None, err
    try:
        clean = text.strip().lstrip("```json").lstrip("```").rstrip("```").strip()
...
Be specific and practical."""
    return call_ai(system, user, max_tokens=2000)


def ai_draft_dsr_response(dsr):
...
- Is properly formatted as a formal letter"""
    return call_ai(system, user, max_tokens=1500)


def ai_generate_privacy_notice(data):
...
def ai_chat(message, regulation=None, history=None):
    ...
    try:
        if AI_PROVIDER == "anthropic":
            ...
            # Fall back to simple call_ai for OpenAI/Gemini
            return call_ai(system, message, max_tokens=1500)
    except Exception as e:
        return None, str(e)


def ai_gap_analysis(regulation_from, regulation_to, activities):
...
6. QUICK WINS — what's already compliant"""
    return call_ai(system, user, max_tokens=2500)
